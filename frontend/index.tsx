...
function Flow() {
  ...
  const [nodes, setNodes, onNodesChange] =
    useNodesState<NodeData>(layoutedNodes);
  const [edges, setEdges, onEdgesChange] = useEdgesState(layoutedEdges);

  const onConnect = useCallback(
    async (params: Connection) => {
      try {
        const { sourceRecord } = await addConnectionToTable(nodes, params, {
  ...
  const onEdgesDelete = useCallback(
    (edgesDelete: Edge<{ sourceRecord: Record }>[]) => {
      edgesDelete.map(async (edge) => {
        ...
        await deleteConnectionsFromTable(edge, record, fieldName, table);
      });
    },
    [nodes]
  );
    ...
    async (_, node: Node) => {
      ...
      setEdges((es) => {
        ...
        let nextEdges = es.filter((e) => {
          if (
            e.source === closeEdge?.source &&
            e.target === closeEdge?.target &&
            e.className !== "temp"
          ) {
            isEdgeExists = true;
          }
        ...
        if (closeEdge && !isEdgeExists) {
          ...
          addConnectionToTable(nodes, closeEdge, {
            ...settingsValidationResult.settings,
          });
          nextEdges.push({
            ...closeEdge,
